...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='AA_RR.')
    add_management_args(parser)
    add_experiment_args(parser)
    add_rehearsal_args(parser)
    parser.add_argument('--Power_alpha', type=float, default=1.0,
                        help='Power for first term')
    parser.add_argument('--Power_beta', type=float, default=0.5,
                        help='Power for second term')
    parser.add_argument('--Power_gamma', type=float, default=0.5,
...
class aarr(ContinualModel):
    NAME = 'aa_rr'
    COMPATIBILITY = ['class-il']

    def __init__(self, backbone, loss, args, transform):
        super(aarr, self).__init__(backbone, loss, args, transform)
    ...
    def begin_task(self, dataset, train_loader):
        ...
        self.class_weights_ = {i: (1.0 if i >= ((self.task - 1)*dataset.N_CLASSES_PER_TASK) 
                                   else (1.0/(self.task - 1))**self.args.Power_alpha * 
                                   (1.0/(self.task - (i // dataset.N_CLASSES_PER_TASK)))**self.args.Power_beta * 
                                   ((1.0/(self.task - 1)) * (self.args.buffer_size/self.n_sample_per_task))**self.args.Power_gamma) 
    ...
    def observe(self, inputs, labels, not_aug_inputs, index_):
        ...
        if self.epoch < self.args.E:
            ...
            self.net.eval()
            with torch.no_grad():
                AA_RR_logits, _____ = self.net.pcrForward(not_aug_inputs)
                __, AA_RR_pred = torch.max(AA_RR_logits.data, 1)
                
                # Accumulate confidences
                for i in range(targets.shape[0]):                    
                    is_correct = int(AA_RR_pred[i] == labels[i])  # 1 if correct, 0 otherwise
                    correct_batch.append(is_correct)
                
                # Record 0/1 correctness values for samples in the corresponding tensor
                correct_tensor = torch.tensor(correct_batch, dtype=torch.float32)                
                self.confidence_by_sample[self.epoch, index_] = correct_tensor
        ...
        if self.buffer.is_empty():
            weighted_loss = WeightedLoss(temperature=0.09, class_weights=self.class_weights_)
