...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Ours_alpha')
...
class Oursalpha(ContinualModel):
    NAME = 'ours_alpha'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(Oursalpha, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, index_):
        ...
        if self.epoch < self.args.E:
            ...
            self.net.eval()
            with torch.no_grad():
                Ours_alpha_logits = self.net(not_aug_inputs)
                __, Ours_alpha_pred = torch.max(Ours_alpha_logits.data, 1)
                
                # Accumulate confidences
                for i in range(targets.shape[0]):                    
                    is_correct = int(Ours_alpha_pred[i] == labels[i])  # 1 if correct, 0 otherwise
