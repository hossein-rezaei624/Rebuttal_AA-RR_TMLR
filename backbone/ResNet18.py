...
class ResNet(MammothBackbone):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
        ...
        if args.model == 'pcr' or args.model == 'ours_beta' or args.model == 'aa_rr':
            self.pcrLinear = cosLinear(nf * 8 * block.expansion, num_classes)
        if args.model == 'delta' or args.model == 'ours_gamma':
            self.head = nn.Linear(nf * 8 * block.expansion, 128)
    ...
    def deltaForward(self, x):
        feat = self.forward(x, 'features')
        feat = F.normalize(self.head(feat), dim=1)
        return feat
        
    def deltaLogits(self, x):
